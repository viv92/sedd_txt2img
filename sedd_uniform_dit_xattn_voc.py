...
# noise schedule
def geometricNoise(t, sigma_min = 1e-3, sigma_max = 1):
    sigma_min, sigma_max = torch.tensor(sigma_min), torch.tensor(sigma_max)
    total_noise = sigma_min ** (1 - t) * sigma_max ** t
    rate_noise = sigma_min ** (1 - t) * sigma_max ** t * (sigma_max.log() - sigma_min.log())
...
# perturbation function for forward diffusion process (uniform case)
def perturb(x, sigma, vocab_size): # x.shape: [batch_size, seq_len]
    sigma = sigma.unsqueeze(-1) # sigma.shape: [batch_size, 1]
    flip_prob = 1 - (-sigma).exp() # note that flip_prob = 1 - exp(-vocab_size * sigma) = 1 - exp(-sigma), i.e., the vocab_size is merged into sigma
    flip_indices = torch.rand(*x.shape, device=x.device) < flip_prob 
    x_perturb = torch.where(flip_indices, torch.randint_like(x, vocab_size), x) 
    return x_perturb # x_perturb.shape: [b, seqlen]


# loss function 
def score_entropy_loss(log_score, sigma, x, x0, vocab_size): # log_score.shape: [b, seqlen, vocab_size]
    ...
    # since ratio = p(y) / p(x) = ( 1 - exp(-N) ) / ( 1 + (N-1) * exp(-N) ) = 1 - ( N / (exp(N) - 1 + N) )
    ratio = 1 - vocab_size / (esigm1 + vocab_size) # ratio.shape: [b * num_flipped_tokens_in_each_sequence]

    ## prepare loss terms (equation 5 in the SEDD paper)

    # negative_term
    # torch.gather gathers the log_scores along the vocab dimension for the token values in the perturbed sequence
    neg_term = log_score.mean(dim=-1) - torch.gather(log_score, -1, x[..., None]).squeeze(-1) / vocab_size
    ...
    # positive term
    score = log_score.exp()
    pos_term = score.mean(dim=-1) - torch.gather(score, -1, x[..., None]).squeeze(-1) / vocab_size
...
def get_stag_score(score, dsigma):
    ...
    stag_score = ((epow - 1) / (dim * epow)) * score.sum(dim=-1, keepdim=True) + score / epow
    return stag_score
...
def get_stag_prob(x, dsigma, vocab_size):
    trans = torch.ones(*x.shape, vocab_size, device=x.device) * (1 - (-dsigma[..., None]).exp()) / vocab_size
    trans = trans.scatter(-1, x[..., None], torch.zeros_like(trans))
    stag_prob = trans.scatter(-1, x[..., None], 1 - trans.sum(dim=-1, keepdim=True))
    return stag_prob


## sampling function
def get_sample(net, seq_len, vocab_size, num_sampling_steps, sample_batch_size, sample_condition, cfg_scale, device, eps=1e-5):
    # x_T
    x = torch.randint(0, vocab_size, (sample_batch_size, seq_len), dtype=torch.int64, device=device)
    flip_counts = torch.zeros_like(x)
    ...
    for i in range(num_sampling_steps):
        ...
        curr_sigma = geometricNoise(t)[0]
        next_sigma = geometricNoise(t - dt)[0]
        dsigma = curr_sigma - next_sigma

        # get conditioned score
        log_score_cond = net(x, curr_sigma.squeeze(-1), sample_condition, seddType='uniform')
        score_cond = log_score_cond.exp().clone().detach()

        # get unconditioned score
        log_score = net(x, curr_sigma.squeeze(-1), None, seddType='uniform')
        ...
        stag_prob = get_stag_prob(x, dsigma, vocab_size)

        # sampling probability for reverse diffusion process - equation 19 of SEDD paper
        probs = stag_score * stag_prob

        # x_(t-1)
        x_t_minus1 =  sample_categorical(probs)

        flipped_idx = torch.where(x != x_t_minus1)
        flip_counts[flipped_idx] += 1
    ...
    sigma = geometricNoise(t)[0]

    log_score_cond = net(x, sigma.squeeze(-1), sample_condition, seddType='uniform')
    score_cond = log_score_cond.exp().clone().detach()

    log_score = net(x, sigma.squeeze(-1), None, seddType='uniform')
    score = log_score.exp().clone().detach()
    ...
    stag_prob = get_stag_prob(x, sigma, vocab_size)
    ...
    flipped_idx = torch.where(x != x_t_minus1)
    flip_counts[flipped_idx] += 1
    flip_counts = (flip_counts - 1).float().mean()

    x = x_t_minus1
        
    return x, flip_counts
...
if __name__ == '__main__':
    ...
    # NOTE: for a future fsq_transformer, pad vocab to increase vocab size to a nice number 
    vocab_size = 1
    ...
    results_dir = './results_seddUniform_voc_xattn_rotaryDisabledXattn/'
    ckpts_dir = './ckpts/'
    ckpt_path = ckpts_dir + 'seddUniform_voc_dit_xattn_rotaryDisabledXattn.pt'
    ...
    while train_step < num_train_steps + train_steps_done:
        ...
        sigma, dsigma = geometricNoise(t)

        # perturb the data
        x_perturb = perturb(x, sigma, vocab_size)

        # use bfloat16 precision for speed up 
        with torch.autocast(device_type='cuda', dtype=torch.bfloat16):

            # get score
            log_score = dit(x_perturb, sigma, condition, seddType='uniform')

            # calculate loss 
            loss = score_entropy_loss(log_score, sigma.unsqueeze(-1), x_perturb, x, vocab_size)
        ...
        if (train_step+1) % sampling_freq == 0: ## sample 
            ...
            with torch.no_grad():
                ...
                x_sample, flip_counts = get_sample(dit, seq_len, vocab_size, num_sampling_steps, sample_batch_size, sample_caption_emb, cfg_scale, device) # x_sample.shape: [b, seqlen]
        ...
        if (train_step+1) % plot_freq == 0: ## save ckpt and plot losses
            ...
            ax[0].plot(losses, label='train_loss')
            ax[0].set_title('final_loss:{:.3f}'.format(losses[-1]))
            ax[0].legend()

            ax[1].plot(flips, label='flip_counts')
            ax[1].set_title('final_value:{:.3f}'.format(flips[-1]))
            ax[1].legend()
