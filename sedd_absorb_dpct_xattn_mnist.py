...
# import DiT 
from utils_sedd_dpct_xattn import *
...
## sampling function
def get_sample(net, seq_len, mask_token, vocab_size, num_sampling_steps, sample_batch_size, sample_condition, device, eps=1e-5):
    # x_T
    ...
    for i in range(num_sampling_steps):
        ...
        log_score = net(x, curr_sigma.squeeze(-1), sample_condition)
    ...
    log_score = net(x, sigma.squeeze(-1), sample_condition)
    score = log_score.exp().clone().detach()
...
if __name__ == '__main__':
    ...
    total_images_in_dataset = 60000
    num_train_steps_per_epoch = total_images_in_dataset // batch_size
    total_train_steps = num_train_steps_per_epoch * num_epochs
    sampling_freq = total_train_steps // 20 # 30
    sample_batch_size = 16
    p_uncond = 1. 
    cfg_scale = 1.5
    plot_freq = total_train_steps // 3 # 5

    results_dir = './results_mnist_dpct_xattn/'
    ...
    max_seq_len = seq_len * 2 + 1 # [t, perturbed_x, final_emb]
    # x_dim = 1
    condition_dim = 1 
    dpct = init_dpct(max_seq_len, x_seq_len, d_model, condition_dim, vocab_size, d_k, d_v, n_heads, n_layers, d_ff, dropout, device).to(device)

    # freeze vqvae
    freeze(vqvae_model)

    # optimizer and loss criterion
    optimizer = torch.optim.AdamW(params=dpct.parameters(), lr=lr)
    ...
    while epoch < num_epochs:
        ...
        for imgs, labels in pbar2:
            ...
            # for sampling 
            sample_condition = condition[:sample_batch_size] # shape: [sample_batch_size, 1, 1]
            ...
            # get score
            log_score = dpct(x_perturb, sigma, condition)
            ...
            torch.nn.utils.clip_grad_norm_(dpct.parameters(), max_norm=1.0)
            ...
            if len(losses) == 0:
                ...
                loss_ema = 0.9 * (losses[-1]) + 0.1 * loss.item()
            ...
            if (train_step+1) % sampling_freq == 0: ## sample 
                ...
                with torch.no_grad():
                    ...
                    x_sample = get_sample(dpct, seq_len, mask_token, vocab_size, num_sampling_steps, sample_batch_size, sample_condition, device) # x_sample.shape: [b, seqlen]
            ...
            if (train_step+1) % plot_freq == 0: ## plot losses
