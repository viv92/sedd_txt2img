...
class DPCT_Embedder(nn.Module):
    def __init__(self, max_seq_len_dpct, x_seq_len, d_model, dropout, pos_enc_fixed, condition_dim, vocab_size, device):
        super().__init__()
...
class DPCT(nn.Module):
    def __init__(self, embedder, decoder, max_seq_len_dpct, x_seq_len, d_model, vocab_size, device):
        super().__init__()
    ...
    def forward(self, x, # x.shape: [b, x_seq_len] 
        ...
        dpct_input_emb, condition_emb = self.embedder(x, t, condition) # dpct_input_emb.shape: [batch_size, max_seq_len_dpct, d_model]
        ...
        out = dpct_out_seq[:, -self.x_seq_len:] # out.shape: [batch_size, x_seq_len, d_model]
...
def init_dpct(max_seq_len_dpct, x_seq_len, d_model, condition_dim, vocab_size, d_k, d_v, n_heads, n_layers, d_ff, dropout, device):
    ...
    dit_embedder = DPCT_Embedder(max_seq_len_dpct, x_seq_len, d_model, dropout, positional_encoder_fixed, condition_dim, vocab_size, device)
    model = DPCT(dit_embedder, decoder, max_seq_len_dpct, x_seq_len, d_model, vocab_size, device)
